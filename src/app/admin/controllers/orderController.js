...
export async function createOrder(req, res) {
  try {
    const {
      projectId, clientId, assignedToId, vendorId, vendorLocationId, vendorHandlerId,
    ...
    return res.status(201).json({ success: true, message: 'Order created', data: { orderId: order.orderId, id: order.id } });
  } catch (error) {
...
export async function updateOrder(req, res) {
  try {
    const { orderId } = req.params;
    const { assignedToId, status, deliveryStatus, vendorId, vendorLocationId, vendorHandlerId, date, time, deliveryAddress } = req.body;
    ...
    const updated = await db.order.update({
      ...
      data: {
        ...
        ...(vendorLocationId !== undefined && { vendorLocationId: vendorLocationId ? parseInt(vendorLocationId) : null }),
        ...(vendorHandlerId !== undefined && { vendorHandlerId: vendorHandlerId ? parseInt(vendorHandlerId) : null }),
        ...(date !== undefined && { date }),
        ...(time !== undefined && { time }),
...
export async function addComment(req, res) {
  try {
    ...
    const notifyTargets = [
      ...
      ...(order.assignedToId ? [{ type: 'FIELD_TECH', id: String(order.assignedToId) }] : []),
    ];
