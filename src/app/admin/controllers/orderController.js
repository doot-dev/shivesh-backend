...
function buildVendorInclude() {
  return {
    ...
    include: {
      vendor: { select: { id: true, companyName: true, ownerName: true, phone: true } },
 ...
 */
async function resolveVendorSelection({ vendorId, vendorLocationId, vendorHandlerId }) {
  ...
  let location = null;
  if (vendorLocationId !== undefined && vendorLocationId !== null && vendorLocationId !== '') {
  ...
  let handler = null;
  if (vendorHandlerId !== undefined && vendorHandlerId !== null && vendorHandlerId !== '') {
    ...
    if (!location || handler.vendorLocationId !== location.id) {
      return { error: 'Vendor handler does not belong to this vendor location' };
    }
  }
...
export async function listOrders(req, res) {
  try {
    const { page = 1, limit = 20, status, clientId, assignedToId, search } = req.query;
    ...
    const where = {
      ...
      ...(status && { status }),
      ...(assignedToId && {
        technicians: { some: { userId: parseInt(assignedToId), isDeleted: false } },
      }),
      ...(clientId && {
        client: { clientId },
      }),
    ...
    const [orders, total] = await Promise.all([
      db.order.findMany({
        where,
        include: {
          project: { select: { projectId: true, projectName: true, siteName: true } },
    ...
    return res.status(200).json({ success: true, data: orders, total, page: pageNum });
  } catch (error) {
...
export async function createOrder(req, res) {
  try {
    const {
      projectId, clientId, vendors = [], technicians = [],
      productName, productGrade, quantity, deliveryAddress, date, time,
    } = req.body;

    if (!projectId || !clientId || !productName || !productGrade || !quantity) {
      return res.status(400).json({ success: false, message: 'projectId, clientId, productName, productGrade and quantity are required' });
    }

    if (!Array.isArray(vendors) || !Array.isArray(technicians)) {
      return res.status(400).json({ success: false, message: 'vendors and technicians must be arrays' });
    }

    const project = await db.project.findFirst({ where: { projectId, isDeleted: false } });
    if (!project) return res.status(404).json({ success: false, message: 'Project not found' });

    const client = await db.client.findFirst({ where: { clientId, isDeleted: false } });
    ...
    for (const v of vendors) {
      if (!v?.vendorId) {
        return res.status(400).json({ success: false, message: 'Each vendor requires a vendorId' });
      }

      const { error, vendor, location, handler } = await resolveVendorSelection(v);
    ...
    const technicianIds = [...new Set(technicians.map((t) => parseInt(t?.userId ?? t)))];
    if (technicianIds.some((id) => Number.isNaN(id))) {
      return res.status(400).json({ success: false, message: 'Each technician requires a numeric userId' });
    }

    if (technicianIds.length) {
      const found = await db.user.findMany({
        where: { id: { in: technicianIds }, role: 'FIELD_TECHNICIAN', isDeleted: false },
        select: { id: true },
      });

      if (found.length !== technicianIds.length) {
        return res.status(404).json({ success: false, message: 'One or more field technicians not found' });
      }
    }
...
export async function updateOrder(req, res) {
  try {
    ...
    const { productName, productGrade, quantity, deliveryAddress, date, time } = req.body;
...
export async function updateOrderStatus(req, res) {
  try {
    ...
    if (!status && !deliveryStatus) {
      return res.status(400).json({ success: false, message: 'status or deliveryStatus is required' });
    }
...
export async function deleteOrder(req, res) {
  try {
    ...
    await db.order.update({ where: { id: order.id }, data: { isDeleted: true } });
...
export const createOrderVendor = async (req, res) => {
  try {
    const { err, status: validationStatus } = await validatorFunction(req.body, createOrderVendorValidation);
...
export const updateOrderVendor = async (req, res) => {
  try {
    const { err, status: validationStatus } = await validatorFunction(req.body, updateOrderVendorValidation);
    ...
    const { orderVendorId, orderId, vendorId, vendorLocationId, vendorHandlerId } = req.body;
    ...
    if (!existing) {
      return res.status(404).json({ success: false, message: 'Order vendor not found' });
    }
    ...
    const { error, vendor, location, handler } = await resolveVendorSelection({
      ...
      vendorLocationId: vendorLocationId !== undefined ? vendorLocationId : existing.vendorLocationId,
      vendorHandlerId: vendorHandlerId !== undefined ? vendorHandlerId : existing.vendorHandlerId,
    });
...
export const deleteOrderVendor = async (req, res) => {
  try {
    ...
    return res.status(200).json({ success: true, message: 'Vendor removed successfully' });
  } catch (error) {
...
export const createOrderTechnician = async (req, res) => {
  try {
    const { err, status: validationStatus } = await validatorFunction(req.body, createOrderTechnicianValidation);
    ...
    const technician = await db.user.findFirst({
      where: { id: parseInt(userId), role: 'FIELD_TECHNICIAN', isDeleted: false },
    });
...
export const updateOrderTechnician = async (req, res) => {
  try {
    const { err, status: validationStatus } = await validatorFunction(req.body, updateOrderTechnicianValidation);
    ...
    logger.info(`Updating technician assignment: ${orderTechnicianId} on order: ${orderId}`);
    ...
    if (!existing) {
      return res.status(404).json({ success: false, message: 'Order technician not found' });
    }
...
export const deleteOrderTechnician = async (req, res) => {
  try {
    ...
    logger.info(`Removing technician: ${orderTechnicianId} from order: ${orderId}`);
